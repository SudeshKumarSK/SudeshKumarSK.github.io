...
// This is for testing in localhost locally.
...
const flaskApi = "http://127.0.0.1:5000/api";
...
async function fetchGeoLocation() {
  ...
  const response = await fetch("https://ipinfo.io/json?token=aebbe99507a8b0");
...
function gmapHref(location) {
  ...
  if (location["line1"])
    completeAddress = completeAddress + location["line1"] + " ";
  if (location["city"])
    completeAddress = completeAddress + location["city"] + " ";
  if (location["state"])
    completeAddress = completeAddress + location["state"] + " ";
  if (location["postalCode"])
    completeAddress = completeAddress + location["postalCode"];
...
function generateVenueDetails(responseData, venueDetailsDiv) {
  ...
  if (responseData["address"]) {
    ...
    city.classList.add("line1");
    city.innerHTML =
      responseData["address"]["city"] + ", " + responseData["address"]["state"];
    ...
    postalCode.classList.add("line1");
...
async function getEventDetails(id, eventName, genre) {
  const url = `${flaskApi}/eventdetails?id=${encodeURIComponent(id)}`;
  await sendJsonData(url)
    ...
    .then((data) => {
      ...
      if (responseData["artist"]["name"]) {
        ...
        // leftContainerArtistContent.style.cursor = "pointer";
      ...
      if (genreString) {
        ...
        leftContainerGenresContent.classList.add(
          "left-container-content-style"
        );
      ...
      if (responseData["ticketStatus"]) {
        ...
        leftContainerTicketStatusContent.classList.add(
          "left-container-content-style-sale"
        );
      ...
      if (responseData["saleUrl"]) {
        ...
        // leftContainerPurchaseContent.style.cursor = "pointer";
      ...
      arrowDown.addEventListener("click", async () => {
        ...
        if (responseData["venue"]) {
          const url = `${flaskApi}/venuedetails?venue=${encodeURIComponent(
            responseData["cityName"] + " " + responseData["venue"]
          )}`;
          await sendJsonData(url)
            .then((res) => res.json())
            .then((data) => {
              ...
              if (data["flag"]) {
                ...
                const eventdetailsDiv =
                  document.querySelector(".event-details-div");
                eventdetailsDiv.classList.add("event-details-enh");
...
function generateDynamicTable(data) {
  if (data["flag"] == false) {
    ...
    for (let i = 0; i < numberOfRows; i++) {
      ...
      cell.innerHTML =
        responseData[i]["localDate"] + "  " + responseData[i]["localTime"];
      ...
      link.addEventListener("click", async () => {
        ...
        if (document.querySelector(".event-details-enh")) {
          document
            ...
            .classList.remove("event-details-enh");
        ...
        if (!inProgressDetail) {
          ...
          await getEventDetails(
            responseData[i]["id"],
            responseData[i]["event"],
            responseData[i]["genre"]
          );
        }
      });
...
form.addEventListener("submit", async (event) => {
  ...
  if (!inProgressEvent) {
    ...
    const url = `${flaskApi}/eventsearch?data=${encodeURIComponent(jsonData)}`;

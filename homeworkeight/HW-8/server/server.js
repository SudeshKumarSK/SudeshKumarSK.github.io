...
const gcp_API_KEY = "AIzaSyCgVhydDpBZzYGBv41BQrC1VecGRkpVzLo";
...
app.get("/api/autocomplete", async (req, res) => {
  ...
  try {
    ...
    const autocompleteResponse = await ticketmaster.performAutocomplete(
      ticketMasterConfig,
      "/suggest"
    );
...
app.get("/api/eventsearch", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const gcpResponse = await ticketmaster.getGeocoding(
        gcpBaseUrl,
        gcpParams
      );
    ...
    ticketMasterConfig["ticketMasterParams"]["segmentId"] =
      eventData["category"];
    ...
    try {
      const eventSearchResponse = await ticketmaster.eventSearch(
        ticketMasterConfig,
        "/events.json?"
      );
...
app.get("/api/eventdetails", async (req, res) => {
  ...
  try {
    ...
    const eventDetailsResponse = await ticketmaster.eventDetails(
      ticketMasterConfig,
      "/events/" + eventId
    );
...
app.get("/api/venuedetails", async (req, res) => {
  ...
  try {
    ...
    const venueDetailsResponse = await ticketmaster.venueDetails(
      ticketMasterConfig,
      "/venues/"
    );
...
app.get("/api/artistDetails", async (req, res) => {
  ...
  try {
    ...
    const artistDetailsResponse = await ticketmaster.artistDetails(
      ticketMasterConfig,
      "/events/" + eventId,
      spotifyApi
    );
    if (artistDetailsResponse["status"]) {
      ...
      throw Error(
        "Couldn't get the artists' details for the event ID being passed!"
      );
    }
